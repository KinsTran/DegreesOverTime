...
GenderChart <- function(gender) {
  ...
  if(is.element("F1", gender)) {
    females.2013.2014 <- females.data[2:37, "2013 14 Total"]
    p <- p %>% add_trace(y = ~females.2013.2014, name = 'Females: 2013-2014', visible = TRUE)
  }
  if(is.element("M1", gender)) {
    males.2013.2014 <- males.data[2:37, "2013 14 Total"]
    p <- p %>% add_trace(y = ~males.2013.2014, name = 'Males: 2013-2014', visible = TRUE)
  } 
  if(is.element("F2", gender)){
    females.2014.2015 <- females.data[2:37, "2014 15 Total"]
    p <- p %>% add_trace(y = ~females.2014.2015, name = 'Females: 2014-2015', visible = TRUE)
  }
  if(is.element("M2", gender)) {
    males.2014.2015 <- males.data[2:37, "2014 15 Total"]
    p <- p %>% add_trace(y = ~males.2014.2015, name = 'Males: 2014-2015', visible = TRUE)
  }
